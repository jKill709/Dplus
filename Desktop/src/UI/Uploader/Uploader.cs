...
namespace Dplus_Desktop.UI.UploaderForm
{
    public partial class Uploader : Form
    {
        ...
        {
            List<ListViewItem>? itemsToProcess = SourceFiles_Box.SelectedItems.Count > 0
                ? new List<ListViewItem>(SourceFiles_Box.SelectedItems.Cast<ListViewItem>())
                : null;
            setControlablilty(false);
            try
            {
                ...
                // Upload files
                await clusters[Clusters_Box.Text].UploadFiles(itemsToProcess);
